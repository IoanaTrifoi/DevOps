...
resource "aws_subnet" "public" {
  count                   = length(var.public_subnets)   # Create several subnets; the count is defined by the length of public_subnets
  vpc_id                  = aws_vpc.main.id              # Attach each subnet to the VPC created earlier
  cidr_block              = var.public_subnets[count.index] # CIDR block for the specific subnet from public_subnets
  ...
  tags = {
    ...
    # ${count.index + 1} adds +1 to the index to get a human-friendly label (1, 2, 3 instead of 0, 1, 2).
    Environment = "lesson-8-9"
  }
}
...
resource "aws_subnet" "private" {
  count             = length(var.private_subnets)   # Create several private subnets; the count matches the length of private_subnets
  ...
  tags = {
    ...
    # ${count.index + 1} is used so that subnet numbering starts at 1.
    Environment = "lesson-8-9"
  }
}
