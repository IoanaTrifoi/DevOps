...
resource "aws_ecr_repository" "ecr" {
  name                 = var.repository_name           # Repository name
  ...
  # Automatic scanning for security vulnerabilities on push
  image_scanning_configuration {
    scan_on_push = var.scan_on_push                    # true → enable
  }

  encryption_configuration {
    encryption_type = "AES256"                         # Or "KMS" + key_id if you need your own key
  }
